...
type LongCountdownTimerProps = {
  duration: number;
};

export const LongCountdownTimer: React.FC<LongCountdownTimerProps> = ({
  duration,
    ...
    useEffect(() => {
      ...
      if (time > 0) {
        document.title = `${formatted} - Long Timer`;
      } else {
      ...
      return () => {
        document.title = "Long session"; // or your default app title
      };
    }, [time]);
    ...
    <div className="text-9xl h-[20rem] w-[22rem] grid place-items-center">
      ...
      <div className="text-3xl block mt-1.5 space-x-3">
        ...
        {!isRunning && time === 0 && (
          <button
          className="bg-indigo-200 text-indigo-900 shadow-inner px-2 py-1 rounded-md transition-colors duration-200 hover:cursor-pointer hover:bg-indigo-300 hover:text-indigo-900 active:bg-indigo-400"
           onClick={handleReset} aria-label="Reset timer">
            Reset
          </button>
        )}
